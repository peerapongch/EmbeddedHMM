...
param_update_rw <- function(param_current,param_formed_current,param_lprob_current,X_current,Y,rw_scale,dim){
  ...
  if(check_domain_phi && check_domain_rho){
    ...
    param_formed_new <- list(F=F,sigma_init=sigma_init,sigma=sigma,sigma_init_inv=sigma_init_inv,sigma_inv=sigma_inv,delta=param_formed_current$delta)
...
lambdaModel2_param5 <- function(ssm,N,L,N.mcmc.param=20,init=NULL,seed=NULL,rw_scale=c(0.5,0.5),rho.init=NULL,phi.init=NULL,delta.init=NULL,checkpoint.name=NULL){
