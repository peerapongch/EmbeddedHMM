...
# save(mcmcs,file='./data/poisson_mcmcs_comparison_2.RData')
# load(file='./data/poisson_mcmcs_comparison_1.RData')
...
# # are all sampling times the same for different inits? Nope all same
# # init1
# system.time(
...
acf(mcmcs[[3]]$X_mcmc[,2,1],lag.max=10000)

profvis(
  actime_out <- ACTime(mcmcs,T,dim=dim,lag.max=10000,tps=0.026)  
)


save(actime_out,file='./data/actime_out_poisson_1_comparison1_lag10000.RData')
ac_time <- actime_out
# ac_time[1,1,1]
