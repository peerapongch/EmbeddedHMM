...
seed <- 1 
rw_scale <- c(0.1)
checkpoint.name <- './data/checkpoint_poisson4_3v1.RData'
system.time(
  lambda_out <- lambdaModel2_param3(ssm_poisson,N,L,N.mcmc.param=N.mcmc.param,
  	seed=seed,rw_scale=rw_scale,checkpoint.name=checkpoint.name)
)

save(lambda_out,file='./data/poisson4_lambda_param_3_v1.RData')
