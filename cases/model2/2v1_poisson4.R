...
seed <- 1 
rw_scale <- c(0.1,0.1)
checkpoint.name <- './data/checkpoint_poisson4_2v1.RData'
system.time(
  lambda_out <- lambdaModel2_param2(ssm_poisson,N,L,N.mcmc.param=N.mcmc.param,
  	seed=seed,rw_scale=rw_scale,checkpoint.name=checkpoint.name)
)

save(lambda_out,file='./data/poisson4_lambda_param_2_v1.RData')
